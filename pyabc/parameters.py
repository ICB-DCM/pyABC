...
class Particle:
    """
        An accepted particle, containing the information that can also be
...
class Population:
    """
    A population is basically a list of particles.
    """
    ...
    def get_weighted_distances(self) -> (list, list):
        if not self._is_normalized:
            self._normalize_weights()
        distances_list = []
        weights_list = []
        for particle in self._list:
            for distance in particle.distance_list:
                ...
                weights_list.append(particle.weight)
        return distances_list, weights_list
