...
def weights(n_per_model, transitions, test_transitions, test_X):
    ...
    """
    bootstr_w_at_test_X = []
    for trans, test_trans, n, X in zip(transitions, test_transitions, n_per_model, test_X):
...
def calc_cv(nr_particles, model_weights, N_BOOTSTR, test_w, transitions, test_X):
    """
    ...
    bootstr_w_at_test_X = [weights(n_per_model, transitions, test_transitions, test_X)
    ...
    model_weighted_variations_at_X = [var * n / n_per_model.sum()
                    for var, n in zip(variations_at_X, n_per_model)]
    point_weighted_var_at_X = [var * w for var, w
                    in zip(model_weighted_variations_at_X, test_w)]
