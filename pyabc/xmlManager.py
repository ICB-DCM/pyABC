...
def xmlGenerator(self,para:dict=None):
    ...
    property = etree.SubElement(cellType, "Property")
    system = etree.SubElement(cellType, "System")
    constant = etree.SubElement(system, "Constant")
    diffEqn = etree.SubElement(system, "DiffEqn")
    expression = etree.SubElement(diffEqn, "Expression")
    ...
    populationArgument = etree.SubElement(cellPopulation, "Population")
    ...
    tree.write("model.xml",pretty_print=True,xml_declaration=True, encoding='UTF-8')
