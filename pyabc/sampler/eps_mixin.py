...
class EPSMixin:
    ...
    def sample_until_n_accepted(  # noqa: C901
        ...
        while True:
            ...
            if (
                ...
                for _ in range(
                    0,
                    np.minimum(self.client_max_jobs, self.client_cores()).astype(int)
                    - len(running_jobs),
                ):
                    job_id_batch = []
                    ...
                    running_jobs.append(
                        self.my_client.submit(full_submit_function, job_id_batch)
                    )
