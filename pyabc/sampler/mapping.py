...
latest/task.html#quick-and-easy-parallelism)
    ...
    def __init__(self, map_=map, mapper_pickles: bool = False):
        ...
        self.pickle, self.unpickle = (
            (identity, identity) if mapper_pickles else (pickle.dumps, pickle.loads)
        )

    def __getstate__(self):
        return (self.pickle, self.unpickle, self.nr_evaluations_, self.sample_factory)

    def __setstate__(self, state):
        (self.pickle, self.unpickle, self.nr_evaluations_, self.sample_factory) = state
    ...
    def sample_until_n_accepted(
        ...
        map_function = functools.partial(self.map_function, sample_simulate_accept)
