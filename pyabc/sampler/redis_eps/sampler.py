...
from .cmd import (SSA, N_EVAL, N_ACC, N_REQ, ALL_ACCEPTED,
                  ...
                  BATCH_SIZE, SLEEP_TIME, IS_PREL, GENERATION)
...
class RedisEvalParallelSampler(Sampler):
    ...
    def sample_until_n_accepted(self,
        ...
        population = prel_sample.get_accepted_population()

        # particles, w = population.get_distribution()
        # create new simulate_one
        prel_transitions = copy.deepcopy(abcsmc.transitions)
        for m in population.get_model_probabilities().keys():
            particles, w= population.get_distribution(m=m)
        ...
        model_probabilities_pd = pd.DataFrame({"p": list(model_probabilities.values())})
        prel_transitions_pdf = abcsmc._create_transition_pdf(t + 1, prel_transitions, model_probabilities_pd)

        prior_pdf = abcsmc._create_prior_pdf()
        ...
        pipeline.delete(N_REQ + "_" + str(t)), np.ndarray
