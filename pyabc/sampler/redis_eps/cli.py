...
@click.option(
    "--password", default=None, type=str, help="Password for a secure connection."
)
@click.option(
    "--processes", default=1, type=int, help="The number of worker processes to start"
)
@click.option(
    "--daemon", default=True, type=bool, help="Create subprocesses in daemon mode."
)
...
def work(
    ...
    procs = [
        Process(
            target=_work, args=(host, port, runtime, password, catch), daemon=daemon
        )
...
def _work(host="localhost", port=6379, runtime="2h", password=None, catch=True):
    np.random.seed()
    random.seed()
...
@click.option("--time", "-t", "t", default=None, type=int, help="Generation t.")
