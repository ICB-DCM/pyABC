...
def work_on_population(redis: StrictRedis,
    ...
    # print("GENERATION= ", pipeline.get(GENERATION).execute()[0])

    t = int(pipeline.get(GENERATION).execute()[0])
    ...
    print("batch_size_b= ",int(batch_size_b.decode()))
    ...
    while int(redis.get(N_ACC + "_" + str(t)).decode()) < n_req \
        ...
        if is_prel is True:
            print("IS_PREL= ",is_prel_b.decode())
            if bool(int(redis.get(IS_PREL + "_" + str(t)).decode())) is False:
                is_prel = False
...
@click.option('--time', default=None, type=int, help='generation.')
...
def _manage(command, host="localhost", port=6379, password=None, time=None):
    ...
    if command == "info":
        pipe.get(N_WORKER+str(t))
        pipe.get(N_EVAL+str(t))
        pipe.get(N_ACC+str(t))
        pipe.get(N_REQ+str(t))
        res = pipe.execute()
