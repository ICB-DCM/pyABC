...
def abc_detail(abc_id):
    ...
    model_probabilities.columns = list(map("{}".format, model_probabilities.columns))
    ...
    if len(model_probabilities) > 0:
        ...
        particles_fig.line(particles["t"], particles["particles"], line_width=2)

        samples_fig = figure()
        samples_fig.xaxis.axis_label = "Generation t"
        samples_fig.yaxis.axis_label = "Samples"
        samples_fig.line(populations["t"], populations["samples"], line_width=2)
...
def server_info():
    history = app.config["HISTORY"]
    return render_template(
        "server_info.html", db_path=history.db_file(), db_size=round(history.db_size, 2)
    )
...
@click.command()
@click.option(
    "--debug", default=False, type=bool, help="Whether to run the server in debug mode"
)
@click.option(
    "--port", default=5000, type=int, help="The port on which the server runs"
)
@click.argument("db")
