...
class NonEmptyMultivariateMultiTypeNormalDistribution:
    ...
    This distribution is essentially a multivariate normal,
    ...
    parameter_types: list
        A list containing ``int`` and/or ``float`` to indicate whether
        a parameter
        is of type ``float`` or ``int``.

    zero_covariance_substitutes: float
        Substitutes zero variance entries on the diagonal of the diagonal
        representation
    ...
    def __init__(self, covariance_matrix: np.ndarray,
        ...
        import warnings
        warnings.warn(
            "Deprecated: NonEmptyMultivariateMultiTypeNormalDistribution")
        # check parameter lengths
        if not (covariance_matrix.shape[0]
                == covariance_matrix.shape[1]
                == len(parameter_names)
                == len(parameter_types)):
            raise Exception("MultivariateMultiTypeNormalDistribution: "
                            "Parameter numbers not matching. "
                            " The shape of the covariance matrix has to "
                            "agree with the length of parameter_names "
                            "and the length of parameter_types.")

        # check for possible singularity of covariance matrix
        #  due to integer data types
        ...
        try:
            ...
            raise ValueError("Bad covariance matrix: {}"
                             .format(covariance_matrix)) from value_error
        self.zero_covariance_substitutes = zero_covariance_substitutes
        self.covariance_matrix = v.dot(
            sp.diag(sp.maximum(w, zero_covariance_substitutes))).dot(v.T)
    ...
    def pdf(self, x: dict) -> float:
        ...
        ranges = [(k-.5, k+.5)
                  for k, _t in zip(parameters_sorted, self.parameter_types)
                  if _t == int]
        ...
        int_par_indices = [k for k in range(len(parameters_sorted))
                           if self.parameter_types[k] == int]
    ...
    def __mul__(self, other):
        ...
        return NonEmptyMultivariateMultiTypeNormalDistribution(
            self.covariance_matrix.copy()*other,
...
def MultivariateMultiTypeNormalDistribution(
        covariance_matrix, parameter_names,  parameter_types,
        zero_covariance_substitutes=1e-4) \
        -> Union[NonEmptyMultivariateMultiTypeNormalDistribution,
                 EmptyMultivariateMultiTypeNormalDistribution]:
        """
        Factory function for multi variate and multi type normal distribution.

        This distribution is essentially a multivariate normal, but takes into
        account
        if a type is an integer and returns it always as rounded integer.
        This is useful if some of your model parameters are discrete.
        ...
        multivariate_distribution: \
           Union[NonEmptyMultivariateMultiTypeNormalDistribution, \
           EmptyMultivariateMultiTypeNormalDistribution]
            Returns NonEmptyMultivariateMultiTypeNormalDistribution
            of len(parameter_names) > 0 otherwise
        ...
        if len(parameter_names) > 0:
            ...
            rv_logger.debug(
                "Create EmptyMultivariateMultiTypeNormalDistribution")
            return EmptyMultivariateMultiTypeNormalDistribution()
