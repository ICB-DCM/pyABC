...
def test_continuous_non_gaussian(db_path, sampler):
    ...
    def f_expected(u):
        return (
            (np.log(u) - np.log(d_observed)) / (-np.log(d_observed)) * (u > d_observed)
        )

    x = np.linspace(0.1, 1)
    max_distribution_difference = np.absolute(f_empirical(x) - f_expected(x)).max()
...
def test_gaussian_multiple_populations_crossval_kde(db_path, sampler):
    ...
    parameter_perturbation_kernels = [
        GridSearchCV(
            MultivariateNormalTransition(), {"scaling": np.logspace(-1, 1.5, 5)}
        )
    ]
    abc = ABCSMC(
        models,
