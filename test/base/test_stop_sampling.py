...
def test_stop_acceptance_rate_too_low(db_path):
    ...
    assert (
        df["acceptance_rate"].iloc[-2] >= set_acc_rate or df["t"].iloc[-2] == -1
...
def test_stop_early(db_path):
    ...
    for sampler in [mc_sampler, sc_sampler]:
        abc = ABCSMC(
            model, Distribution(par=st.uniform(0, 10)), dist, pop_size, sampler=sampler
        )
        abc.new(db_path, {"par": 0.5})
        history = abc.run(max_nr_populations=8, min_acceptance_rate=set_acc_rate)
        df = history.get_all_populations()
