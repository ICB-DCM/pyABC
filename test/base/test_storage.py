...
def rand_pop_list(m: int):
    ...
    pop = [
        Particle(
            m=m,
            parameter=Parameter({"a": np.random.randint(10), "b": np.random.randn()}),
...
def test_single_particle_save_load(history: History):
    particle_list = [
        Particle(
            ...
            weight=0.2,
            accepted_sum_stats=[{"ss": 0.1}],
            accepted_distances=[0.1],
        ),
    ]
...
def test_save_no_sum_stats(history: History):
    ...
    for _ in range(0, 6):
        particle = Particle(
            ...
            weight=0.2,
            accepted_sum_stats=[{"ss0": np.random.random(), "ss1": np.random.random()}],
...
def test_get_population(history: History):
    ...
    # distances
    distances = sum((p.accepted_distances for p in population.get_list()), [])
    distances_h = sum((p.accepted_distances for p in population_h.get_list()), [])
...
def test_sum_stats_save_load(history: History):
    ...
    particle_list = [
        Particle(
            ...
            weight=0.2,
            accepted_sum_stats=[
                {"ss1": 0.1, "ss2": arr2, "ss3": example_df(), "rdf0": r["iris"]}
            ],
            accepted_distances=[0.1],
        ),
        Particle(
            ...
            weight=0.2,
            accepted_sum_stats=[
                {"ss12": 0.11, "ss22": arr, "ss33": example_df(), "rdf": r["mtcars"]}
            ],
            accepted_distances=[0.1],
        ),
    ]
        ...
        for t in range(4):
            ...
            for m in range(5):
                ...
                for particle in pops[(h, m, t)]:
                    particle_list.append(particle)
            h.append_population(t, 0.1, Population(particle_list), 2, model_names)
...
def test_population_retrieval(history: History):
    history.append_population(1, 0.23, Population(rand_pop_list(0)), 234, ["m1"])
    history.append_population(2, 0.123, Population(rand_pop_list(0)), 345, ["m1"])
    history.append_population(
...
def test_observed_sum_stats(history_uninitialized: History, gt_model):
    ...
    obs_sum_stats = {"s1": 1, "s2": 1.1, "s3": np.array(0.1), "s4": np.random.rand(10)}
...
def test_population_to_df(history: History):
    ...
    for t in range(3):
        for m in range(4):
            history.append_population(
                t, 0.23, Population(rand_pop_list(m)), 234, ["m0", "m1", "m2", "m3"]
            )
