...
db_path = "sqlite:///" + os.path.join(tempfile.gettempdir(), "test_visualize.db")
...
def test_sample_numbers():
    pyabc.visualization.plot_sample_numbers(histories, rotation=43, size=(5, 5))
    _, ax = plt.subplots()
...
def test_kdes():
    ...
    pyabc.visualization.plot_kde_matrix_highlevel(history, height=27.43, refval=p_true)
    plt.close()
